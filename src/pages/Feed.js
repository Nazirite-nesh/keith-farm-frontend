...
function Feed() {
  ...
  const addFeed = async () => {
    if (editId) {
      ...
      await axios.post(`${API}/api/feed`, {...newFeed, createdBy: userName});
    }
  ...
  const handleEdit = (item) => {
    setEditId(item._id);
    setNewFeed({ name: item.name, unit: item.unit, reorderLevel: item.reorderLevel });
    window.scrollTo(0, 0);
  };
  ...
  const updateStock = async () => {
    ...
    await axios.post(`${API}/api/feed/update`, { feedId: selected, type: action, quantity: Number(qty) });
    setQty("");
    fetchData();
  };
    ...
    <div>
      ...
      <div style={styles.container}>
          ...
          <table style={styles.table}>
            <thead>
              <tr style={styles.thead}>
                <th>Feed</th><th>Stock</th><th>Unit</th><th>Status</th><th>By</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {items.map(i => (
                <tr key={i._id} style={styles.row}>
                  ...
                  <td>{i.quantity}</td>
                  <td>{i.unit}</td>
                  <td>{i.quantity === 0 ? "🔴 Out" : i.quantity <= i.reorderLevel ? "🟡 Low" : "🟢 OK"}</td>
                  <td style={{fontSize:12,color:"#555"}}>{i.createdBy || "-"}</td>
                  <td>
                    <button style={styles.editBtn} onClick={() => handleEdit(i)}>Edit</button>
                    {isAdmin && <button style={styles.deleteBtn} onClick={() => handleDelete(i._id)}>Delete</button>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div style={styles.form}>
          <input style={styles.input} placeholder="Feed Name" value={newFeed.name} onChange={(e) => setNewFeed({...newFeed, name: e.target.value})} />
          <input style={styles.input} placeholder="Unit (bags/kg)" value={newFeed.unit} onChange={(e) => setNewFeed({...newFeed, unit: e.target.value})} />
          <input style={styles.input} type="number" placeholder="Reorder Level" value={newFeed.reorderLevel} onChange={(e) => setNewFeed({...newFeed, reorderLevel: e.target.value})} />
          <button style={styles.button} onClick={addFeed}>{editId ? "Update Feed" : "Add Feed Type"}</button>
          {editId && <button style={styles.cancelBtn} onClick={() => { setEditId(null); setNewFeed({ name: "", unit: "", reorderLevel: "" }); }}>Cancel</button>}
        </div>

        <h3 style={styles.subtitle}>Update Stock</h3>
        <div style={styles.form}>
          <select style={styles.input} onChange={(e) => setSelected(e.target.value)}>
            <option value="">Select Feed</option>
            {items.map(i => (<option key={i._id} value={i._id}>{i.name}</option>))}
          </select>
          <select style={styles.input} onChange={(e) => setAction(e.target.value)}>
